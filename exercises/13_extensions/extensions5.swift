...
func main() {
	...
	test("`throwError()` is validly implemented in an `Error` extension") {
		...
		do {
			...
			assertTrue(implemented, "Make sure to implement the `throwError()` function in a `Error` extension.")
			assertFalse(true,"Make sure that you are throwing to exit the function.")
		} catch(let e: MyError) {
